...
public class DeactivateDataRetentionPolicy extends Function {
  ...
  public void invoke(
    ...
    String dataRetentionPolicy =
        ((TextValue) dataRetentionPolicyResult.getValue(RETENTION_POLICY_TYPE).get())
            .getString()
            .orElseThrow(() -> new ContractContextException("Retention policy type not found"));

    Key putClusteringKey =
        new Key(
            new BigIntValue(CREATED_AT, createdAt),
            new TextValue(RETENTION_POLICY_TYPE, dataRetentionPolicy),
            new TextValue(ORGANIZATION_ID, contractArgument.getString(ORGANIZATION_ID)));
    Put put =
        new Put(partitionKey, putClusteringKey)
