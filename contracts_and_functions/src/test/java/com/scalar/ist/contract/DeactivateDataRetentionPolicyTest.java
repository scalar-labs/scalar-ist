...
public class DeactivateDataRetentionPolicyTest {
  private static final String MOCKED_ASSET_NAME = "data_retention_policy";
  private static final String SCHEMA_FILENAME = "deactivate_data_retention_policy.json";
  ...
  private DeactivateDataRetentionPolicy deactivateDataRetentionPolicy;

  @BeforeEach
  private void setUp() {
    MockitoAnnotations.initMocks(this);
    deactivateDataRetentionPolicy = spy(new DeactivateDataRetentionPolicy());
  }
  ...
  public void invoke_AsProcessor_ProperArgumentsAndPropertiesGiven_ShouldPass() {
    ...
    when(deactivateDataRetentionPolicy.getCertificateKey()).thenReturn(certificateKey);
    when(deactivateDataRetentionPolicy.getCertificateKey().getHolderId())
        .thenReturn(MOCKED_HOLDER_ID);
    doReturn(userProfile)
        .when(deactivateDataRetentionPolicy)
    ...
    // Act
    deactivateDataRetentionPolicy.invoke(ledger, argument, Optional.of(properties));

    // Assert
    verify(deactivateDataRetentionPolicy)
        .invokeSubContract(GET_USER_PROFILE, ledger, userProfileArgument);
    verify(deactivateDataRetentionPolicy)
        .invokeSubContract(PUT_ASSET_RECORD, ledger, putRecordArgument);
    verify(deactivateDataRetentionPolicy)
    ...
    verify(deactivateDataRetentionPolicy)
        .invokeSubContract(VALIDATE_PERMISSION, ledger, permissionValidationArgument);
    verify(deactivateDataRetentionPolicy, times(5)).invokeSubContract(any(), any(), any());
  }
  ...
  public void invoke_PropertyWithoutSchema_ShouldThrowContractContextException() {
    ...
    verify(deactivateDataRetentionPolicy, never()).invokeSubContract(any(), any(), any());
  }
  ...
  public void invoke_WithWrongHolderId_ShouldThrowContractContextException() {
    ...
    assertThatThrownBy(
            () -> {
              deactivateDataRetentionPolicy.invoke(
    ...
    verify(deactivateDataRetentionPolicy)
        .invokeSubContract(VALIDATE_ARGUMENT, ledger, validateArgumentArgument);
    verify(deactivateDataRetentionPolicy).invokeSubContract(any(), any(), any());
  }
  ...
  private JsonObject preparePutRecordArgument(JsonObject arguments, JsonObject properties) {
    ...
    String assetId =
        String.join("-", assetName, arguments.getString(ORGANIZATION_ID), createdAt.toString());
