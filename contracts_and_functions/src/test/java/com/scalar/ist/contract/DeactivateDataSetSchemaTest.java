...
public class DeactivateDataSetSchemaTest {
  private static final String MOCKED_ASSET_NAME = "data_set_schema";
  private static final String SCHEMA_FILENAME = "deactivate_data_set_schema.json";
  ...
  private DeactivateDataSetSchema deactivateDataSetSchema;

  @BeforeEach
  private void setUp() {
    MockitoAnnotations.initMocks(this);
    deactivateDataSetSchema = spy(new DeactivateDataSetSchema());
  }

  @Test
  public void invoke_AsProcessor_ProperArgumentsAndPropertiesGiven_ShouldPass() {
    invoke_WithProferArgumentsForGivenRole(ROLE_PROCESSOR);
  }

  @Test
  public void invoke_AsController_ProperArgumentsAndPropertiesGiven_ShouldPass() {
    invoke_WithProferArgumentsForGivenRole(ROLE_CONTROLLER);
  }

  private void invoke_WithProferArgumentsForGivenRole(String role) {
    // Arrange
    ...
    JsonObject userProfile = prepareUserProfile(role);
    JsonObject dataRetentionPolicy = prepareDataRetentionPolicy();
    JsonObject dataRetentionPolicyArgument = prepareDataRetentionPolicyArgument();
    JsonObject permissionValidationArgument =
        preparePermissionValidationArgument(userProfile, dataRetentionPolicy);
    ...
    when(deactivateDataSetSchema.getCertificateKey()).thenReturn(certificateKey);
    when(deactivateDataSetSchema.getCertificateKey().getHolderId()).thenReturn(MOCKED_HOLDER_ID);
    doReturn(userProfile)
        .when(deactivateDataSetSchema)
    ...
    doReturn(dataRetentionPolicy)
        .when(deactivateDataSetSchema)
        .invokeSubContract(GET_ASSET_RECORD, ledger, dataRetentionPolicyArgument);
    doReturn(null)
        .when(deactivateDataSetSchema)
        .invokeSubContract(VALIDATE_ARGUMENT, ledger, validateArgumentArgument);

    // Act
    deactivateDataSetSchema.invoke(ledger, argument, Optional.of(properties));

    // Assert
    verify(deactivateDataSetSchema)
        .invokeSubContract(GET_USER_PROFILE, ledger, userProfileArgument);
    verify(deactivateDataSetSchema).invokeSubContract(PUT_ASSET_RECORD, ledger, putRecordArgument);
    verify(deactivateDataSetSchema)
        .invokeSubContract(VALIDATE_ARGUMENT, ledger, validateArgumentArgument);
    verify(deactivateDataSetSchema)
        .invokeSubContract(VALIDATE_PERMISSION, ledger, permissionValidationArgument);
    verify(deactivateDataSetSchema, times(5)).invokeSubContract(any(), any(), any());
  }
  ...
  public void invoke_PropertyWithoutSchema_ShouldThrowContractContextException() {
    ...
    verify(deactivateDataSetSchema, never()).invokeSubContract(any(), any(), any());
  }
  ...
  public void invoke_WithWrongHolderId_ShouldThrowContractContextException() {
    ...
    assertThatThrownBy(
            () -> {
              deactivateDataSetSchema.invoke(
    ...
    verify(deactivateDataSetSchema)
        .invokeSubContract(VALIDATE_ARGUMENT, ledger, validateArgumentArgument);
    verify(deactivateDataSetSchema).invokeSubContract(any(), any(), any());
  }
  ...
  private JsonObject prepareDataRetentionPolicy() {
    return Json.createObjectBuilder().add(ORGANIZATION_ID, MOCKED_ORGANIZATION_ID).build();
  }

  private JsonObject prepareDataRetentionPolicyArgument() {
  ...
  private JsonObject preparePermissionValidationArgument(
      JsonObject userProfile, JsonObject dataRetentionPolicy) {
    ...
    return Json.createObjectBuilder()
        ...
        .add(
            ...
            Json.createArrayBuilder().add(dataRetentionPolicy.getString(ORGANIZATION_ID)).build())
        .build();
  }
